...
=====================================
Changes from v3:
  - Season-capped lookback — rolling windows reset at season boundaries
    so last season's form never bleeds into this season's features.
    Historical seasons (22-23, 23-24) now serve purely as training
    volume; they teach the model what patterns matter without
    contaminating current-season form statistics.
  - predict_match() also filters to current season only for form lookup
"""
...
class PLPredictor:
    ...
    def _build_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        def _rolling_team_stats(df: pd.DataFrame, window: int) -> pd.DataFrame:
            ...
            Prior-season matches are discounted by prior_season_weight (default 0.4)
            before rolling — so last season is a background signal, not an equal vote.
            Windows cross season boundaries naturally; the discount handles the rest.
            """
            ...
            # Discount prior-season matches before rolling
            current_season = combined['season'].max()
            weight = combined['season'].apply(
                lambda s: 1.0 if s == current_season else self.prior_season_weight
            )
            combined['pts'] *= weight
            combined['gf'] *= weight
            combined['ga'] *= weight
        ...
        for side, team_col in [('h', 'home_team'), ('a', 'away_team')]:
            for prefix, stats_df in [('short', short), ('long', long)]:
                merged = df[['date', team_col]].merge(
                    stats_df, left_on=['date', team_col],
    ...
    def train_model(self, include_historical: bool = False):
        ...
        fig = px.bar(
            ...
            title='Feature Importance (XGBoost v4)',
    ...
    def predict_match(self, home_team: str, away_team: str):
        ...
        # Use all data but prior-season matches are discounted via prior_season_weight
        # applied during training — no hard cap needed here
